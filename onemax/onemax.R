library(ggplot2)
library(data.table)
library(dplyr)
library(reticulate)

###
py_run_file(file = "/MAI5030/onemax/onemax.py")

data.table(Geracão = 1:100, cga = py$progress_cga, sga = py$progress_sga) %>%
  melt.data.table(id.vars = 'Geracão', variable.name = "Algorítimo", value.name = "Ajuste") %>%
  ggplot(aes(x = Geracão, y = Ajuste, color = Algorítimo)) + 
  geom_line() + 
  scale_color_brewer(palette = "Set1")

###
py_run_file(file = "/MAI5030/onemax/onemax_comp.py")

for(i in 1:length(py$sizes)){
  sga_acc = round(sum(py$fitness_sga[[i]]==py$global_fitness)/py$tests, digits = 3)*100
  cga_acc = round(sum(py$fitness_cga[[i]]==py$global_fitness)/py$tests, digits = 3)*100
  
  print(
    data.table(Execuções = 1:py$tests, cga = py$fitness_cga[[i]], sga = py$fitness_sga[[i]]) %>%
      melt.data.table(id.vars = 'Execuções', variable.name = "Acurácia", value.name = "Ajuste") %>%
      mutate(Acurácia = if_else(Acurácia=="cga", paste0(Acurácia, " - ", cga_acc, "%"), paste0(Acurácia, " - ", sga_acc, "%"))) %>%
      ggplot(aes(x = Execuções, y = Ajuste, color = Acurácia)) + 
      ylim(c(0,20)) +
      geom_line() + 
      scale_color_brewer(palette = "Set1") +
      ggtitle(paste0("População de tamanho ", py$sizes[i]))
    )
}

